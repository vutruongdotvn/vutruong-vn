...
export default function IntroWidget() {
  ...
  const renderInlineParts = (text: string) => {
    ...
    return inlineParts.map((part, partIndex) => {
      if (part.type === "bold") {
        return <strong key={partIndex} className="font-semibold text-foreground">{part.value}</strong>;
      }
      if (part.type === "link") {
        return (
          <Link
            ...
            className="text-sky-700 dark:text-sky-300 font-medium hover:text-sky-900 dark:hover:text-sky-300 break-words"
      ...
      if (part.type === "hashtag") {
        ...
        return (
          <Link
            ...
            className="text-sky-700 dark:text-sky-300 font-medium hover:underline active:scale-95 inline-flex break-words"
            onClick={(e) => e.stopPropagation()}
          >
            {part.value}
          </Link>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {isAdmin && !editing && (
          <button
            ...
            className="text-xs text-muted-foreground hover:text-foreground cursor-pointer active:scale-95"
          >
      ...
      {!editing && (
        loadingProfile ? (
          <div className="space-y-1.5 sm:space-y-3 animate-pulse">
            <div className="h-[1rem] bg-muted rounded-full w-full"></div>
            <div className="h-[1rem] bg-muted rounded-full w-3/4"></div>
            <div className="h-[1rem] bg-muted rounded-full w-1/3"></div>
          </div>
        ) : (
          <div className="text-[.9375rem]/6 text-foreground space-y-3">
            {isCollapsed ? (
              ...
              <div className="space-y-2">
                ...
                {/*isLong && (
                  <button
                    ...
                    className="mt-1 align-baseline font-medium text-muted-foreground hover:text-foreground cursor-pointer text-xs transition"
                  >
            ...
            <div className="border-t border-border pt-3 space-y-1 lg:space-y-2 text-[.9375rem]/6 text-foreground">
              <div className="flex items-center gap-2">
                <span className="flex items-center justify-center xt-sm sm:text-lg bg-muted text-center rounded-full size-8"><i className="far fa-blog" /></span> <b>Trang</b> <span>•</span> <span>Blog cá nhân</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="flex items-center justify-center xt-sm sm:text-lg bg-muted text-center rounded-full size-8"><i className="far fa-clock" /></span> <b>Tham gia</b> <span>•</span> <span>Tháng 7 năm 2017</span>
              </div>
            </div>
          </div>
      ...
      {editing && (
        <div className="space-y-2">
          <div
            ...
            className="w-full text-sm sm:text-base/6 outline-none whitespace-pre-wrap break-words empty:before:content-[attr(data-placeholder)] empty:before:text-muted-foreground"
          ...
          <div className="flex justify-end gap-1 mt-4 pt-4 border-t border-border">
            <button
              onClick={handleCancel}
              className="text-xs px-5 py-2 rounded-lg bg-muted hover:bg-secondary cursor-pointer active:scale-95"
            >
              Huỷ
            </button>
            <button
              ...
              className="text-xs px-5 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground cursor-pointer active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            >
