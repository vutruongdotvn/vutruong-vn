...
import { getPostRouteState } from "@/lib/getPostRouteState";
import {
...
export async function getPostMetadata(id: string): Promise<Metadata> {
  // Chỉ tạo metadata đầy đủ cho bài public. Bài privacy và ID không tồn tại
  // cố ý có cùng metadata noindex để không tạo oracle tiết lộ sự tồn tại.
  const publicRouteState = await getPostRouteState(id);
  if (!publicRouteState) return NOT_FOUND_METADATA;
